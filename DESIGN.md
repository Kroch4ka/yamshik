...
- **HTTP**: Faraday. Ядро отдаёт плагинам сконфигурированный `Yamshik::HTTP::Client`: ретраи с backoff, circuit breaker (своя реализация, closed/open/half-open), таймауты, логгер, маппинг сетевых сбоев в иерархию исключений.
  - **Точки подстройки под причуды службы** (у части КС «недоступность» приходит в 200 с телом-ошибкой, а 500 может означать бизнес-ошибку):
    - `classifier:` у клиента — решает судьбу ответа (успех / вернуть адаптеру как бизнес-ответ / инфра-исключение). Дефолт — `HTTP::StatusClassifier` по статусам. Вызывается внутри breaker'а → инфра-исключения из кастомного классификатора автоматически кормят CB и ретраи.
    - `count_failure:` у `HTTP::CircuitBreaker` — предикат, какие исключения считать симптомом недоступности. Дефолт — только инфра-исключения ядра (таймауты, connection, 5xx, 429); программные ошибки breaker не открывают.
    - `idempotent:` у вызова — безопасность ретрая (§3).
